...
def compute_quality_checks(source_text: str, translated_text: str) -> Optional[QualityCheckResult]:
    ...
    target_numbers = NUMBER_PATTERN.findall(normalized_target)
...
EXAMPLE_TEXTS: Dict[str, str] = {
    "Select an Example": "",
    "Example 1": "فلما صح عنده بفطرته الفائقة التي تنبهت لمثل هذه الحجة؛ أن جسم السماء متناهٍ، أراد أن يعرف على أي شكل هو وكيفية انقطاعه بالسطوح التي تحده.",
    "Example 2": "ربما طرقها العدو أو أسرع الجند إليها، وتداعوا نحوها حتى يكاد يترامى ذلك بهم إلى انتهاب العسكر ثوران الفتنة. فإن أهل الفتن، وسيئ السيرة، ومن همته الشرك كثير، ومسارعتهم إلى الخير بعيدة.",
    "Example 3": "قَالَ يَا قَوْمِ أَرَأَيْتُمْ إِن كُنتُ عَلَىٰ بَيِّنَةٍ مِّن رَّبِّي وَآتَانِي رَحْمَةً مِّنْ عِندِهِ فَعُمِّيَتْ عَلَيْكُمْ أَنُلْزِمُكُمُوهَا وَأَنتُمْ لَهَا كَارِهُونَ",
    "Example 4": "ولا يخفى أن الحمام من أسرع المواصلات؛ فإن الحمام يقطع مسيرة عشرين يوماً فى بعض يوم. فقد حكى ابن سعيد فى كتابه \"حيا المحل وجنى النحل\" أن الوزير أبا الفرج يعقوب بن كلس، وزير العزيز أحد خلفاء الفاطميين",
    # ...
}
                     ...
                     if st.session_state.editable_gaps: # Check if list is not empty
                          ...
                          for i, gap_dict in enumerate(st.session_state.editable_gaps):
                               ...
                               st.markdown(f'<span style="display:inline-block; width: 12px; height: 12px; background-color:{gap_color}; border-radius: 50%; margin-right: 8px;"></span>'
                                     f'**{i+1}. {gap_dict["name"]}**', unsafe_allow_html=True)
