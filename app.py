...
st.set_page_config(
    page_title="Classical Arabic Translator",
    page_icon="📖",
    layout="wide",
...
# Initialize the LLM client and orchestrator once
# @st.cache_resource # <-- REMOVE CACHING
def get_services():
    print("Initializing services...") # Add print to see when this runs
    llm_client = LLMClient()
    orchestrator = TranslationOrchestrator(llm_client)
    return orchestrator

# translation_orchestrator = get_services() # <-- REMOVE initial call here
...
with st.container(border=True):
    ...
    translate_button = st.button("Translate & Analyze", type="primary", use_container_width=True)
