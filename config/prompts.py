...
Your task is to analyze the provided Arabic text and its English translation to identify **ALL significant cultural translation gaps**. A cultural gap occurs where concepts, references, values, social norms, idioms, or material culture from the source text do not have a direct or easily understood equivalent in the target language and culture (modern English). Focus on elements requiring cultural context beyond simple lexical equivalence. Important: The gap SHOULD NOT be the entire translation, but must be a specific segment (word, phrase, expression, etc.).
